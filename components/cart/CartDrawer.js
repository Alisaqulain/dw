    ...
    <>
      ...
      <div className="fixed inset-y-0 right-0 z-[70] flex w-full max-w-md flex-col bg-white shadow-2xl animate-slide-in-right">
        <div className="flex items-center justify-between border-b border-slate-100 px-5 py-4">
          <div>
            <h2 className="text-lg font-bold text-slate-900">{t("yourBag")}</h2>
            <p className="text-xs text-slate-500">{cartCount} {cartCount !== 1 ? t("items") : t("item")}</p>
          </div>
        ...
        {cart.length === 0 ? (
          <div className="flex flex-1 flex-col items-center justify-center px-6 text-center">
            ...
            <p className="mt-4 font-semibold text-slate-800">{t("bagEmpty")}</p>
            <p className="mt-1 text-sm text-slate-500">{t("discoverProducts")}</p>
          ...
          <>
                  ...
                  <div key={item.lineId || item.productId} className="flex gap-3 rounded-2xl bg-slate-50 p-3">
                    ...
                    <div className="flex flex-1 flex-col">
                      ...
                      <div className="mt-auto flex items-center justify-between">
                        ...
                        <button onClick={() => removeFromCart(item.lineId || item.productId)} className="text-xs text-red-500 hover:text-red-600">{t("remove")}</button>
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="border-t border-slate-100 bg-white p-5">
              {cartSavings > 0 && (
                <p className="mb-2 text-center text-xs font-medium text-emerald-600">{t("youSave", { amount: formatPrice(cartSavings) })}</p>
              )}
              <div className="space-y-1.5 text-sm">
                <div className="flex justify-between text-slate-600"><span>{t("subtotal")}</span><span>{formatPrice(cartTotal)}</span></div>
                <div className="flex justify-between text-slate-600"><span>{t("delivery")}</span><span>{delivery === 0 ? t("freeDelivery") : formatPrice(delivery)}</span></div>
                <div className="flex justify-between pt-2 text-base font-bold text-slate-900"><span>{t("total")}</span><span>{formatPrice(cartTotal + delivery)}</span></div>
              </div>
              <button
