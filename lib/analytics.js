...
/** Generic CTA click — GA4 + server log */
export function trackCTA(action, label = "") {
  trackServerEvent("cta_click", { productSlug: label, productName: action });
  trackGA4("select_content", {
    content_type: "cta",
    item_id: action,
    item_name: label || action,
  });
  if (typeof window !== "undefined" && window.fbq) {
    window.fbq("trackCustom", "CTAClick", { action, label });
  }
}

/** WhatsApp button clicks */
export function trackWhatsAppClick(context = "floating") {
  trackServerEvent("whatsapp_click", { page: typeof window !== "undefined" ? window.location.pathname : "", productName: context });
  trackGA4("whatsapp_click", { event_category: "engagement", event_label: context });
  if (typeof window !== "undefined" && window.fbq) {
    window.fbq("trackCustom", "WhatsAppClick", { context });
  }
}

/** Product card / link click */
export function trackProductClick(product) {
  if (!product) return;
  trackCTA("product_click", product.slug || product.name);
