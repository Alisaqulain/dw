...
export async function storeImage(base64Data) {
  ...
  let result = await uploadToVercelBlob(buffer, contentType);

  if (!result && process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
    try {
      const dataUri = `data:${contentType};base64,${buffer.toString("base64")}`;
      const cloud = await uploadToCloudinary(dataUri);
      result = { ...cloud, storage: "cloudinary" };
    } catch (err) {
  ...
  if (!result) {
    throw new Error("Image upload failed. Check server logs.");
  }
  ...
  if (storage === "vercel-blob" || publicId.startsWith("products/")) {
    ...
    if (token) {
      try {
        ...
        // pathname may need full URL — try url delete if stored as url
      }
    }
  }
